...
Frontier FrontierSearch::buildNewFrontier(unsigned int initial_cell, unsigned int reference, geometry_msgs::Pose pose, std::vector<bool>& frontier_flag)
{
  ...
  unsigned int idxAreaNumber = whichArea(initial_cell, frontierDiv);
  ...
  while (!bfs.empty()) {
    ...
    for (unsigned int nbr : nhood8(idx, *costmap_)) {
      ...
      if (isNewFrontierCell(nbr, frontier_flag) && whichArea(nbr,frontierDiv)==idxAreaNumber) {
