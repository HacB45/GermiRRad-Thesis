...
//     nav_msgs::OccupancyGrid occGridDataObs;
//     const size_t num_occGridDataObs = occGridDataObs.size();
//     unsigned int width = occGridDataObs.info.width;
//     unsigned int height= occGridDataObs.info.height;
...
//       // save the coordinate until reach an obstacle
//       for (unsigned int h = 0; h < height; ++h)
//       {
//         for (unsigned int w = 0; w < width;  ++w)
//         {
//           char value = occGridDataObs.data[w + h*width];
//           if (value == 100) {
//               return;
//           }
//           else
...
void poseCallback(const geometry_msgs::PoseWithCovarianceStamped::ConstPtr& msg)
{
  ...
  uvAngleLeftLimit = dirAngle + (openAngle/2);
  uvAngleRightLimit = dirAngle - (openAngle/2);
